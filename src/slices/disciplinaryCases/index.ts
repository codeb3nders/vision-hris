...
import { URL_DISCIPLINARY_CASES } from 'constants/EndpointPath';
...
export const createAction: any = createAsyncThunk(
  ...
  async (data: { body: any; access_token: string }) => {
    try {
      ...
      return await createEndpoint(URL_DISCIPLINARY_CASES, data.body, config);
    } catch (err: any) {
...
export const getAllDataAction: any = createAsyncThunk(
  ...
  async (data: { access_token: string, params?: any }) => {
    try {
      ...
      const response = await getByParamsEndpoint(URL_DISCIPLINARY_CASES, config, data.params);
...
export const getByEmployeeNoAction: any = createAsyncThunk(
  ...
  async (data: { access_token: string, employeeNo: string }) => {
    try {
      ...
      const response = await getByEmployeeEndpoint(URL_DISCIPLINARY_CASES, config, data.employeeNo);
      return response.data;
    } catch (err: any) {
...
export const updateAction: any = createAsyncThunk(
  ...
  async (data: { params: any; access_token: string }) => {
      try {
          ...
          return await updateEndpoint(URL_DISCIPLINARY_CASES, data.params, config);
      } catch (err: any) {
...
export const deleteAction: any = createAsyncThunk(
  ...
  async (data: { id: string; access_token: string }) => {
    try {
      ...
      return await deleteEndpoint(URL_DISCIPLINARY_CASES, data.id, config);
    } catch (err: any) {
...
export const disciplinaryCaseSlice = createSlice({
  name,
  initialState: {
    ...initialCreateState,
    ...initialUpdateState,
...
export const { reset } = disciplinaryCaseSlice.actions;

export default disciplinaryCaseSlice.reducer;
