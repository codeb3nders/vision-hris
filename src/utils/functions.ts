...
export function generateCompanyEmail(fName, lName, rank, isRehire = false) {
  ...
  if (rank.toLowerCase() === "rank and file") {
    return `${firstName[0][0]}${lName.replace(
      " ",
      ""
    )}${ctr}.vcdcph@gmail.com`.toLowerCase();
  } else {
    return `${firstName[0]}.${lName.replace(
      " ",
      ""
    )}${ctr}@vcdcph.com`.toLowerCase();
  }
}

export function getProbationaryEndDate(dateHired) {
  return moment(dateHired).endOf("day").add(6, "months").endOf("day");
}
