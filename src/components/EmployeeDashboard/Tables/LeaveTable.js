...
const LeaveTable = ({isApprover, isOT}) => {
	...
	const statusCell = cell =>
		isHRLogin || isApprover ? (
			<Chip
				size="small"
				color={cell.value === "Pending" ? "warning" : cell.value === "Approve" ? "success" : "error"}
				label={
					<Select
						fullWidth
						onChange={() => {}}
	...
	const leaveColumns = [
		{
			...
			renderCell: cell => {
				return (
					<Button title={cell.value} variant="text" onClick={() => setViewDetails({details: cell, status: true})}>
						{cell.value}
					</Button>
				)
			}
	...
	return (
		<>
			<ViewDetailsModal viewDetails={viewDetails} setViewDetails={setViewDetails} isApprover={isApprover} isOT={isOT} />
			<Snackbar
				open={openSnack}
			...
			>
				<Alert onClose={handleClose} severity="success" sx={{width: "100%"}}>
					{LeaveTypes.filter(lt => lt.id === selectedLeaveType)[0]?.label} {isOT ? "OT" : "Leave"} Application submitted
					successfully.
				</Alert>
			</Snackbar>
			<Grid item>
				<Breadcrumbs aria-label="breadcrumb" sx={{my: 2, justifyContent: "center", display: "flex"}}>
					<Typography color="inherit">ESS {isApprover ? "Management" : "Application"}</Typography>
					{newForm ? (
						<Link underline="hover" color="inherit" href="#" onClick={() => setNewForm(false)}>
							{isOT ? "OT" : "Leave"} Applications
						</Link>
					) : (
						<Typography color="text.primary">{isOT ? "OT" : "Leave"} Applications</Typography>
					)}
					{newForm && <Typography color="text.primary">New Application</Typography>}
				</Breadcrumbs>
			</Grid>
			...
			{newForm ? (
				<Grid container justifyContent="center">
					{isOT ? (
						<OTForm setNewForm={setNewForm} setSelectedLeaveType={setSelectedLeaveType} setOpenSnack={setOpenSnack} />
					) : (
						<LeaveForm
				...
				<Card sx={{width: "100%", p: 2}}>
					{!isApprover && (
						<div style={{marginBottom: 16, textAlign: "right"}}>
							<Button startIcon={<AddCircleOutlineTwoTone />} onClick={() => setNewForm(true)}>
					...
					<div style={{height: 400, width: "100%"}}>
						<DataGrid
							...
							onCellDoubleClick={(params, event, details) => console.log({params, event, details})}
						/>
					</div>
				</Card>
			)}
